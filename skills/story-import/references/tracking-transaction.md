...
{PYTHON} {当前 skill 根}/scripts/tracking_commit.py rebuild --project {书项目根}
```
...
- `rebuild`：只读取 schema 4 `_tracking-state.json`，重建全部派生视图并删除孤儿角色快照；不改 state、`state_revision` 或逐章记录，第 0 章项目也可直接使用。
...
校验失败与写入失败处理方式不同：校验失败（字段非法、退役结构、容量超限）要按报错改事务本身，重跑同一份结果不变。派生视图被手改、缺失或出现孤儿角色快照，导致 `check` 报 `derived view ...` / `character snapshot files differ` 时，运行 `tracking_commit.py rebuild --project {书项目根}` 从权威 state 整份重建，再运行 `check`；不需要伪造 `mode=revision` 事务，也不手改派生文件或删除 `_tracking-state.json`。手写出的逐章记录会让同章 `append` 永久报 `chapter delta N already exists with different content`——删掉那个手写文件后重跑原事务即可。
