...
- 先读取 `.story-deployed` 并执行顶部 Spawn 版本提示；版本不匹配只报告 Notice，不单独阻断 agent 调用。
- 按 `.codex/story-agents/chapter-extractor.md` → `.codex/agents/chapter-extractor.toml` 检查 Phase 2 长篇并行 agent；文件缺失或运行时不可用时才降级串行。

**部署标记缺失或当前 Codex 的 agent 不可用时**，提示用户：

> 「检测到当前项目尚未部署可用的写作 agent。建议先运行 `$story-setup` 再回来导入，否则深度分析阶段会降级为串行处理。」

给用户两个选择：

1. **先去 setup**：暂停导入，运行 `$story-setup`，部署完成并新开 Codex 会话后重新触发 `$story-import`；
...
story-long-analyze 在 Stage 0+1（黄金三章）后会**自动停靠**并用 直接向用户确认 询问是否继续全量拆解（对应 story-long-analyze 的「Stage 1 停靠点」）。但导入场景需要 Stage 2-6 的全套产物（逐章摘要 / 聚合分析 / `剧情/节奏.md` / `剧情/情绪模块.md` / 设定关系 / 汇总报告 / 文风），缺一不可——否则 Phase 3 迁移会拿到半成品。
...
- 可选验证：如果项目已部署 story-explorer agent（优先检查 `.codex/story-agents/` 下的 `story-explorer.md` 是否存在；不存在时检查 `.codex/agents/`），可 spawn `spawn_agent(agent_type="story-explorer", prompt: "项目目录：{dir}\n查询类型：progress\n查询参数：导入验证")` 交叉验证迁移数据完整性
