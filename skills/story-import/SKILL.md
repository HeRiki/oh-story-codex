...
- 检查项目是否已部署 story-setup 基础设施（`.story-deployed` 是否存在）。如不存在，将 `/story-setup` 标记为下一步必需操作（包括 agents、hooks、rules、AGENTS.md）
- 可选验证：如果项目已部署 story-explorer agent（检查 `.codex/story-agents/story-explorer.md` 是否存在），可 spawn `spawn_agent(role: "story-explorer", prompt: "项目目录：{dir}\n查询类型：progress\n查询参数：导入验证")` 交叉验证迁移数据完整性
