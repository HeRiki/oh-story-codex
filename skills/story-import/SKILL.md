...
story-long-analyze 在 Stage 0+1（黄金三章）后会**自动停靠**并用 直接向用户确认 询问是否继续全量拆解（对应 story-long-analyze 的「Stage 1 停靠点」）。但导入场景需要 Stage 2-5 的全套产物（逐章摘要 / 聚合分析 / 设定关系 / 汇总报告），缺一不可——否则 Phase 3 迁移会拿到半成品。
