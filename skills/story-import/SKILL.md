...
| 2 | 逐章摘要 | 章节文本 | 章节摘要.md（含情节点+角色）。每章3-40情节点（密度150-200字/个，按字数动态调节）。角色过滤（龙套不提取、别名归类）。**读取 chapter-extractor 参考提示词逐章处理；默认本线程批处理，用户明确要求委派时才用 spawn_agent**。**计数验证：摘要数 == 章节数**。 | 所有章节处理完成 |
...
沿用 story-long-analyze 的分块策略（Stage 2 使用 chapter-extractor 参考提示词逐章处理，其他阶段按以下策略分块）：
...
- 检查项目是否已部署 story-setup 基础设施（`.story-deployed` 是否存在）。如不存在，建议用户运行 `/story-setup` 完成环境部署（包括 `AGENTS.md`、story rules、story agents 参考提示词和上下文模板）
- 可选验证：如果项目已部署 story-explorer 参考提示词（检查 `.codex/story-agents/story-explorer.md` 是否存在），读取该文件并执行 `项目目录：{dir}\n查询类型：progress\n查询参数：导入验证`，交叉验证迁移数据完整性。只有用户明确要求委派时，才把该验证作为 `spawn_agent` 子任务执行。
...
| 部署模板 | `../story-setup/references/templates/AGENTS.md.tmpl` |
