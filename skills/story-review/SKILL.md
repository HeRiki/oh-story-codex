...
路径解析顺序：
1. `{项目根}/skills/{规范路径}`（本仓库开发环境或项目内安装）
2. Codex 全局 skills 目录中同名 `{skill-name}/...` 目录
3. 通过 Glob/Grep 搜索 `*/{规范路径}`
...
如果某工具只接受当前目录相对路径，先尝试 `skills/story-review/references/{file}`；找不到再用 Glob/Grep 搜索 `*/story-review/references/{file}`；禁止改读其他 skill 的 references。
...
4. 识别目标平台（检查 `.active-book` 或用户指定），加载对应 rubric：
   - 番茄小说 → 读取 `story-review/references/rubrics/fanqie.md`（优先 `skills/story-review/...`，其次 Codex 全局 skills 目录）
   - 起点 → 读取 `story-review/references/rubrics/qidian.md`（优先 `skills/story-review/...`，其次 Codex 全局 skills 目录）
   - 知乎盐言 → 读取 `story-review/references/rubrics/zhihu.md`（优先 `skills/story-review/...`，其次 Codex 全局 skills 目录）
   - 未指定 → 默认加载 `story-review/references/quality-rubric.md`（优先 `skills/story-review/...`，其次 Codex 全局 skills 目录）

**Phase 1.5：可选 story-explorer 预查询**。如果项目已部署 story-explorer agent（检查 `.codex/story-agents/story-explorer.md` 是否存在），可 spawn `spawn_agent(name="story-explorer", prompt: "项目目录：{dir}\n查询类型：setting_appearances\n查询参数：{审查涉及的设定关键词}")` 预查设定摘要，将结果注入各 agent 的 prompt，减少重复 grep。此步可选，跳过不影响审查流程。
...
- 提示指令：
  ...
  相关文件路径：{设定/大纲/细纲文件路径}
  参考文件路径规则：以下路径以 skill 名开头；优先从 `{项目根}/skills/` 拼接解析，其次从 Codex 全局 skills 目录查找；不要读取裸文件名，不要跨 skill 读取 references。
