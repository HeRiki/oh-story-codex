...
const WINDOWS_REPLACE_COMMAND = [
  ...
  "$OutputEncoding = $utf8",
  "[System.IO.File]::Replace($env:OH_STORY_REPLACE_SOURCE, $env:OH_STORY_REPLACE_TARGET, $env:OH_STORY_REPLACE_BACKUP, $true)",
].join("; ");
...
export async function replaceFileAtomically(
  ...
  content,
  mode,
  { platform = process.platform, powershellExecutable = "powershell.exe" } = {},
) {
  const temporary = resolve(dirname(target), `.${basename(target)}.story-dashboard-${randomUUID()}.tmp`);
  const backup = resolve(dirname(target), `.${basename(target)}.story-dashboard-${randomUUID()}.bak`);
      ...
      await new Promise((accept, reject) => {
        const child = spawn(
          ...
          ],
          {
            env: {
              ...process.env,
              OH_STORY_REPLACE_BACKUP: backup,
              OH_STORY_REPLACE_SOURCE: temporary,
              OH_STORY_REPLACE_TARGET: target,
            },
        ...
        child.once("close", (code) => {
          if (code === 0) accept();
          else reject(new Error(`Windows 原子替换失败（exit ${code}）：${stderr.trim()}`));
        });
      });
      // 替换成功即以 target 为准；备份删不掉时保留，便于人工恢复。
...
function parseCliArguments(argv) {
  ...
  if (!LOOPBACK_HOSTS.has(options.host)) {
    throw new DashboardError(
      400,
      "loopback_binding_required",
      "Dashboard 只允许绑定本机回环地址，不能把写作文件暴露到局域网",
    );
  }
  return options;
}
...
async function main() {
  ...
  const server = createDashboardServer({ root: workspace });
