...
> Codex 中优先使用 `$story-*` 或 `/skills` 触发。下表以 slash command 展示；在 Codex 里可将 `/story-long-write` 等价替换为 `$story-long-write`。
...
5. Dashboard 默认只监听 `127.0.0.1`；也接受显式回环地址 `localhost` 或 `::1`，不提供局域网/公网模式。不要主动增加 `--allow-network` 或任何远程绑定入口。
...
1. 分析用户请求，提取意图关键词
2. 匹配上表，找到对应的 skill
3. 如果能明确匹配，显式使用对应的 Codex skill，或在本线程按该 skill 的流程执行
...
「查故事资料」「查资料」走 agent 前先做轻量可用性检查（路由只做这一层，不承担全局部署策略）：当前不在子代理上下文、Codex 子代理调用能力可用、且 `.codex/agents/{story-explorer|story-researcher}.toml` 存在 → 可尝试用 `spawn_agent(agent_type=...)` 调用。任一不满足，或 Codex 运行时返回 `unknown agent_type` / 未暴露 custom-agent registry，则降级，不硬失败：
...
2. **最新版本**：优先 `gh release view --json tagName,name,url -R HeRiki/oh-story-codex` 取 `tagName`；无 gh 用 `curl -fsS --max-time 5 https://api.github.com/repos/HeRiki/oh-story-codex/releases/latest` 取 `.tag_name`（jq 或 grep）。查不到 → 告知"暂时拉不到最新版本，可手动看 [Releases](https://github.com/HeRiki/oh-story-codex/releases)"，不报错。
3. **比较**：去掉 `v` 前缀按语义版本比（major.minor.patch）。`gh release` 默认取 latest 稳定版，不含 pre-release。
4. **告知**：
   - 已最新 → 「已是最新版 vX.Y.Z」。
   - 有新版 → 列出 当前 vA → 最新 vB + [Releases](https://github.com/HeRiki/oh-story-codex/releases)/[CHANGELOG](https://github.com/HeRiki/oh-story-codex/blob/master/CHANGELOG.md)（能拿到 release notes 就附本次要点），再用 直接向用户确认 问「现在更新吗？」：
     - 选更新 → 跑 `从本仓 Codex 移植版仓库同步或安装最新版本`（`-g` 全局，去掉则只更当前目录）；完成后提示：已部署过的项目在项目根重跑 `/story-setup`（Codex 中用 `$story-setup`）同步 hooks/agents/references，并**新开一个会话**让 agents 重新注册。
     - 选先不 → 不动，告知随时可再来。
