...
const PLACEHOLDER_PATTERNS = [
  ...
  { re: /(?:此处|以下|这里|下文|后续)\s*(?:省略|略去|略过)(?:[^。！？!?\n]{0,12})/, label: '占位符（裸露省略）', hard: true },
  { re: /(未完待续|TODO|占位符|placeholder)/i, label: '占位符', hard: true },
...
function isDialogueLike(trimmed) {
  return /[“”"'‘’「」『』【】]/.test(trimmed)
    || /^.{0,24}(?:说|道|问|喊|叫|答|笑|骂|喃喃|低声|冷声|吼|叹)[：:]/.test(trimmed);
}
...
function stripDialogue(text) {
  const withoutQuoted = stripQuoted(text);
  return withoutQuoted.replace(
    /^(.{0,24}(?:说|道|问|喊|叫|答|笑|骂|喃喃|低声|冷声|吼|叹)[：:]).*$/,
    '$1',
  );
}

function isIndexInsideQuotedSpan(text, index) {
  const pairs = [
    ['「', '」'],
    ['『', '』'],
    ['【', '】'],
  ...
  for (let i = 1; i < body.length; i += 1) {
    if (
      ...
      (
        ...
        (!/^\s*[“"'‘「『【]/.test(body[i].trimmed) && visibleLength(body[i].trimmed) >= ADJACENT_MIN_LEN)
      )
    ) {
      findings.push({
        line: body[i].lineNo,
...
function findTruncation(content) {
  ...
  // A finished chapter needs sentence-ending punctuation; closing quotes/brackets may trail it.
  if (/(?:[。！？!?]|…{2,}|\.{3,})[…"'”’』」）)】]*$/.test(last.trimmed)) return [];
  return [{
    line: last.lineNo,
...
function findPlaceholders(content) {
  ...
  for (const { trimmed, lineNo } of content) {
    ...
    for (const { re, label, hard } of PLACEHOLDER_PATTERNS) {
      const scanText = !hard && dialogue ? stripDialogue(trimmed) : trimmed;
      if (!scanText.trim()) continue; // 纯台词/纯引用里的 soft 拒绝语可能是正常角色台词，豁免
      const m = re.exec(scanText);
      if (m) {
        const excerpt = scanText === trimmed
          ? trimmed.slice(Math.max(0, (m.index || 0) - 4), (m.index || 0) + 20)
          : trimmed;
        findings.push({
          line: lineNo,
          column: (m.index || 0) + 1,
...
function findMetaLeak(content) {
  ...
  for (const { trimmed, lineNo } of content) {
    ...
    const tier1Matches = [...trimmed.matchAll(new RegExp(META_TIER1_RE.source, 'g'))];
    if (tier1Matches.length) {
      const unquotedMatch = tier1Matches.find((match) => !isIndexInsideDialogueText(trimmed, match.index || 0));
      const m = unquotedMatch || tier1Matches[0];
      const inDialogueText = !unquotedMatch;
    ...
    const m = META_TIER2_RE.exec(trimmed);
    if (m) {
      findings.push({
        line: lineNo,
        column: m.index + 1,
...
function isIndexInsideDialogueText(text, index) {
  if (isIndexInsideQuotedSpan(text, index)) return true;
  const colonDialogue = /^.{0,24}(?:说|道|问|喊|叫|答|笑|骂|喃喃|低声|冷声|吼|叹)[：:]/.exec(text);
  return Boolean(colonDialogue && index >= colonDialogue[0].length);
}
