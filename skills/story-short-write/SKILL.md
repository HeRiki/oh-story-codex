...
正文写作阶段，可读取 `.codex/story-agents/narrative-writer.md` 作为正文写作参考提示词。默认由主线程直接写作；只有用户明确要求委派时才使用 `spawn_agent`。写作时要结合核心框架、小节大纲、涉及角色、对标/拆文路径和本场景拆文召回摘要。
...
**字数统计必须跨平台可执行：优先使用 Python 字符统计**：`python3 -c "from pathlib import Path; print(len(Path('文件路径').read_text(encoding='utf-8')))"`。Windows / DeepSeek / Codex 组合下不要让模型自行估算字数；`wc -m` 仅作为 macOS/Linux 备选，禁止使用 `wc -c`（字节数）。
