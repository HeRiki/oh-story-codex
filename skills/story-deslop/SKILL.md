...
#### 协作提示：narrative-writer（去 AI 味执行）

Phase 2 诊断完成后，优先检查 `.codex/story-agents/narrative-writer.md` 是否存在。若存在，读取该文件作为去 AI 味执行角色参考，并在本线程按诊断结果执行 6 Gate + 三遍法；只有用户明确要求多代理、并行处理或委派时，才使用 `spawn_agent` 分派该任务。

无论本线程执行还是委派执行，以下 Gate 规则都必须遵循。
