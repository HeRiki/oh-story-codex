...
Phase 2 诊断完成后，优先检查 `.codex/story-agents/narrative-writer.md` 是否存在。若存在，读取该文件作为去 AI 味执行角色参考，并在本线程按诊断结果执行 6 Gate + 三遍法 + 问题模式目录；只有用户明确要求多代理、并行处理或委派时，才使用 `spawn_agent` 分派该任务。新增问题模式都归入 Gate A-F 的对应处理；相邻段重复表达同一信息/动作/情绪时，按 Gate C/D 合并去重；字数不足补新子事件或对话。
