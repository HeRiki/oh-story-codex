...
**可选：使用 story-explorer 参考提示词批量加载上下文**。如果存在 `.codex/story-agents/story-explorer.md`，可读取该文件作为 `context_load` 查询的角色说明，一次获取全部写作上下文。默认由主线程执行；只有用户明确要求委派时才使用 `spawn_agent`。如果返回不完整，回退到下方手动加载。
...
**资料研究（按需）**：如果写作中遇到需要查证的外部事实（历史年代、地理方位、职业细节等），暂停写作，读取 `.codex/story-agents/story-researcher.md` 作为研究参考提示词，搜索并输出到 `参考资料/` 目录。研究完成后再继续写作；只有用户明确要求委派时才使用 `spawn_agent`。
