...
**可选：使用 story-explorer 参考提示词批量加载上下文**。如果项目已部署 `.codex/story-agents/story-explorer.md`，读取该文件并执行 `context_load` 查询，一次获取全部写作上下文。默认由主线程执行；只有用户明确要求委派时才使用 `spawn_agent`。如果返回不完整，回退到下方手动加载。
