...
**可选：使用 story-explorer 参考提示词批量加载上下文**。如果项目已部署 story-explorer 参考提示词（检查 `.codex/story-agents/story-explorer.md` 是否存在），读取该文件并执行 `context_load` 查询：`项目目录：{dir}\n查询类型：context_load\n查询参数：准备写第 {N} 章`，一次获取全部写作上下文。如果返回不完整，回退到下方手动加载。
...
**资料研究（按需）**：如果写作中遇到需要查证的外部事实（历史年代、地理方位、职业细节等），暂停写作，读取 `.codex/story-agents/story-researcher.md` 作为研究角色说明并输出到 `参考资料/` 目录；只有用户明确要求委派时才用 `spawn_agent`。研究完成后再继续写作。
