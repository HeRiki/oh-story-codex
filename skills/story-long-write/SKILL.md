...
> 运行环境兼容性：Codex 是内置适配目标。检查专业 agent 时按 `.codex/story-agents/{agent}.md` → `.codex/agents/{agent}.toml` 查找；只有文件缺失、Codex 返回 `unknown agent_type`，或当前运行时未暴露 custom-agent registry 时才降级为 solo/direct，并报告 fallback。
>
...
| 设定/文风.md（自定义文风·优先级最高） | 本书 | 用户自写（Codex 可代写）；导入/拆解不覆盖 | Phase 4 每章写作前：含实质内容则取代对标文风作权威风格基 |
...
5. **伏笔/时间线文件缺失** → 视为当前语义检查点损坏（仅派生视图层），停止写正文；先运行 `tracking_commit.py rebuild --project {书项目根}` 从权威 state 重建，再运行 `check`。卷纲/大纲中的计划不能代替已发生事实的当前检查点。
