...
确认选题方向后，可读取 `.codex/story-agents/story-architect.md` 作为架构参考提示词，辅助题材分析和核心梗设计。默认由主线程执行；只有用户明确要求多代理/委派时，才把该任务作为 `spawn_agent` 子任务执行。
...
4. **资料研究**（按需）：如果写作中遇到需要查证的外部事实（历史年代、地理方位、职业细节等），读取 `.codex/story-agents/story-researcher.md` 作为研究参考提示词，搜索并输出到 `参考资料/` 目录。研究完成后再继续写作；只有用户明确要求委派时才使用 `spawn_agent`。
5. **写作**：读取 `.codex/story-agents/narrative-writer.md`（如存在）作为正文写作参考提示词，结合准备层输出、情绪目标、涉及角色、参考技法和对标召回摘要，输出写入 `正文/第XXX章_章名.md`。写作硬约束：按三维度织入写场景，但仍必须按镜头断段；一段只承载一个动作/信息变化，优先一段一句，避免一段到底。输出前做密度重排：段落 >60 字按句号/动作转折拆开，单句 >45 字拆短。字数硬约束：本章必须达到细纲中设定的字数目标（从细纲读取），优先使用 Python 字符统计；macOS/Linux 可用 `wc -m` 备选；禁止使用 `wc -c` 或模型估算。字数未达标禁止结束本章。
...
质量检查阶段，可读取 `.codex/story-agents/consistency-checker.md` 作为一致性检查参考提示词，获取 S1-S4 分级报告。默认由主线程参照 `quality-checklist.md` 直接检查；只有用户明确要求委派时才使用 `spawn_agent`。

#### Agent 调用：narrative-writer（去AI味审查）

质量检查阶段，可读取 `.codex/story-agents/narrative-writer.md` 作为文字质量审查和去 AI 味检查参考提示词。默认由主线程直接执行；只有用户明确要求委派时才使用 `spawn_agent`。
