...
确认选题方向后，可读取 `.codex/story-agents/story-architect.md` 作为架构参考提示词辅助题材分析和核心梗设计。默认由主线程执行；只有用户明确要求多代理/委派时，才把该任务作为 `spawn_agent` 子任务执行。
...
核心设定阶段，可按需读取以下 `.codex/story-agents/` 参考提示词辅助本线程执行；只有用户明确要求委派时才使用 `spawn_agent`：
- `story-architect.md` — 辅助世界观和核心冲突设计
- `character-designer.md` — 辅助角色设定和语言风格档案
...
大纲搭建阶段，可读取 `.codex/story-agents/story-architect.md` 辅助大纲排布、钩子/反转/情绪弧线设计。默认由主线程直接执行；只有用户明确要求委派时才使用 `spawn_agent`。
...
2. **读取上下文**（按需加载，缺失则跳过。可选快捷路径：如果存在 `.codex/story-agents/story-explorer.md`，可读取该参考提示词执行 `context_load` 查询，一次获取上下文；只有用户明确要求委派时才使用 `spawn_agent`）：
...
3.5. **资料研究**（按需）：如果写作中遇到需要查证的外部事实（历史年代、地理方位、职业细节等），读取 `.codex/story-agents/story-researcher.md` 作为研究参考提示词，搜索并输出到 `参考资料/` 目录。研究完成后再继续写作；只有用户明确要求委派时才使用 `spawn_agent`。
4. **写作**：读取 `.codex/story-agents/narrative-writer.md`（如存在）作为正文写作参考提示词，输出写入 `正文/第XXX章_章名.md`。字数硬约束：本章必须达到细纲中设定的字数目标（从细纲读取），使用 `wc -m` 统计字数，不要用 `wc -c`。字数未达标禁止结束本章。
...
质量检查阶段，可读取 `.codex/story-agents/consistency-checker.md` 执行一致性检查，获取 S1-S4 分级报告；如果参考提示词不存在，由主线程参照 `quality-checklist.md` 直接检查。只有用户明确要求委派时才使用 `spawn_agent`。

#### Agent 调用：narrative-writer（去AI味审查）

文字质量审查和去AI味检查可读取 `.codex/story-agents/narrative-writer.md` 辅助执行；默认主线程处理，只有用户明确要求委派时才使用 `spawn_agent`。
