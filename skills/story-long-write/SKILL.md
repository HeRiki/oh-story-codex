...
> 运行环境兼容性：Codex 是内置适配目标；NarraFork、Web AI、自定义 Agent 等能读取项目文件的环境，可按本 skill 执行长篇流程。检查专业 agent 时按 `.codex/story-agents/{agent}.md` → `.codex/agents/{agent}.toml` 查找；找不到、或 Codex 返回 `unknown agent_type` 时，直接 solo/direct 执行并报告 fallback。
...
若已部署 story-architect agent（优先检查 `.codex/story-agents/story-architect.md`），可让它辅助：
...
3. **写前准备**（下面的 3 步是核心方法在单章写作中的落地：筛选状态 → 召回模块 → 确认意图）：
   ...
   - 3.2 **模块召回、题材卡与文风召回**：
     ...
     - 检查顺序：`.codex/story-agents/story-explorer.md` → `.codex/agents/`。
...
7. **正文执行**：
   - 先检查 narrative-writer agent：`.codex/story-agents/narrative-writer.md` → `.codex/agents/`。
   - 如可用，spawn `spawn_agent(name="narrative-writer", prompt: ...)`，prompt 只传本章必需材料：
