...
EVENT_PATTERN = "(?:" + "|".join(re.escape(event) for event in MANAGED_EVENTS) + ")"
...
def runs_posix_launcher(command: str) -> bool:
    launcher = r"(?:\"[^\"]*\.codex/hooks/run-story-hook\.sh\"|'[^']*\.codex/hooks/run-story-hook\.sh'|[^\s;]*\.codex/hooks/run-story-hook\.sh)"
    shell = r"(?:command\s+)?(?:/[^\s;]*/)?sh\s+(?:--\s+)?"
    suffix = r"(?:\s+(?:\d*>+|<+)\s*[^;\s]+)*\s*(?:;|$)"
    return bool(re.search(rf"(?:^|(?:&&|\|\||[;&])\s*){shell}{launcher}\s+{EVENT_PATTERN}{suffix}", command))


def runs_windows_launcher(command: str) -> bool:
    assigned = bool(re.search(r"\$launcher\s*=\s*join-path\s+[^;]+['\"][^'\"]*\.codex/hooks/run-story-hook\.cmd['\"]", command))
    invoked = bool(re.search(rf"(?:^|[;&{{]\s*)&\s+\$launcher\s+['\"]?{EVENT_PATTERN}['\"]?(?:\s*(?:;|$))", command))
    suffix = r"(?:\s+(?:\d*>+|<+)\s*[^;\s]+)*\s*(?:;|$)"
    direct = bool(re.search(rf"(?:^|(?:&&|\|\||[;&])\s*|call\s+)(?:\"[^\"]*\.codex/hooks/run-story-hook\.cmd\"|'[^']*\.codex/hooks/run-story-hook\.cmd'|[^\s;]*\.codex/hooks/run-story-hook\.cmd)\s+{EVENT_PATTERN}{suffix}", command))
    return (assigned and invoked) or direct


def runs_legacy_python_hook(command: str) -> bool:
    has_hook_path = "story_codex_hook.py" in command and ".codex" in command and "hooks" in command
    if not has_hook_path:
        return False
    event_argument = rf"(?:^|\s){EVENT_PATTERN}(?:\s*(?:2>[^\s]+)?\s*(?:\|\||&&|;|$))"
    if not re.search(event_argument, command):
        return False
    python_dispatch = re.search(
        r"(?:^|(?:&&|\|\||[;&])\s*|then\s+|else\s+|if\s+exist\s+[^\s]+\s+)(?:python3?|py)(?:\s+-\d+)?\s+(?:-c\s+|\"?\$hook\"?|[^\s;]*story_codex_hook\.py)",
        command,
    )
    return bool(python_dispatch and ("runpy.run_path" in command or any(marker in command for marker in MANAGED_COMMAND_MARKERS)))
...
def is_story_setup_hook(hook: object) -> bool:
    ...
    return any(
        runs_posix_launcher(command)
        or runs_windows_launcher(command)
        or runs_legacy_python_hook(command)
