...
def resolve_hook_cwd(value: str) -> Path | None:
    direct = resolve_existing_dir(value)
    if direct is not None:
        return direct
    normalized = value.replace("\\", "/")
    raw_root = os.environ.get("CODEX_PROJECT_DIR")
    if raw_root:
        raw_normalized = raw_root.replace("\\", "/").rstrip("/")
        native_root = resolve_existing_dir(raw_root)
        if native_root is not None:
            if normalized == raw_normalized or normalized.startswith(raw_normalized + "/"):
                rel = normalized[len(raw_normalized):].lstrip("/")
                candidate = native_root.joinpath(*PurePosixPath(rel).parts) if rel else native_root
                return candidate.resolve()
            if value.startswith("/"):
                parts = PurePosixPath(normalized).parts
                for index, part in enumerate(parts):
                    if part != native_root.name:
                        continue
                    rel_parts = parts[index + 1:]
                    candidate = native_root.joinpath(*rel_parts) if rel_parts else native_root
                    return candidate.resolve()
    return None
...
def project_root() -> Path:
    value = os.environ.get("CODEX_PROJECT_DIR")
    if value:
        candidate = resolve_existing_dir(value)
...
def _is_absolute_declaration(value: str) -> bool:
    normalized = value.replace("\\", "/")
...
def read_active_book(root: Path) -> Path | None:
    ...
    if active_file.exists():
        ...
        if declared:
            if _is_absolute_declaration(declared):
                return None
...
def active_book_diagnostic(root: Path) -> str | None:
    active_file = root / ".active-book"
    ...
    try:
        ...
        return "[story context] .active-book 无法读取；修正前不会自动切换到其他书目。"
    declared = lines[0].strip() if lines else ""
    if not declared:
        return None
    if read_active_book(root) is not None:
        return None
    return "[story context] .active-book 无效：必须填写项目根内的相对真实目录；修正前不会自动切换到其他书目。"
...
# JS/Python 两端共享同一规则规格；Codex 项目 Hook 的行为由 test-codex-hooks.sh 覆盖。
...
# 章尾状态总结体：与 trailer-ending 共用文末窗口，盖章过去而非预告将来。
    ...
    *,
    ...
    if context_revision != revision:
        ...
        return (
            ...
            "运行 tracking_commit.py rebuild --project {书项目根} 从 _tracking-state.json 重建派生视图，再运行 check"
        )
...
def session_start() -> None:
    ...
    if book:
        ...
        diagnostic = active_book_diagnostic(root)
        if diagnostic:
...
def extract_apply_patch_targets(command: str) -> list[str]:
    # 与 JS 共享核 extractPatchTargets 保持同一语义，test-codex-hooks.sh 覆盖 Add/Update/Move。
    # 只认 Add/Update 会漏掉 `*** Move to:`——它是 Update File 段的子指令
...
def target_paths_from_hook(obj: dict[str, Any]) -> list[Path]:
    ...
    if not isinstance(payload, str):
        payload = tool_input.get("patch")
    if isinstance(payload, str):
        if tool_name.rsplit(".", 1)[-1] == "Bash":
            raw_targets.extend(extract_prose_targets_from_command(payload))
        else:
            raw_targets.extend(extract_apply_patch_targets(payload))
            raw_targets.extend(extract_prose_targets_from_command(payload))
...
def _is_character_sheet_path(rel: str) -> bool:
    ...
    「正文硬编码角色属性」真警告埋掉。判定口径与本 Codex hook 的提交提醒
    ...
    与 JS core isCharacterSheetPath 保持同一实现规格。"""
    segments = rel.split("/")
    last = len(segments) - 1
...
def staged_markdown_warnings(root: Path) -> str:
    ...
    for raw in proc.stdout.split(b"\0"):
        ...
        # 匹配语义与警告文案遵循当前 Codex hook 合同：name 字段 re.I 大小写不敏感、中文文案。
...
def stop_event(obj: dict[str, Any]) -> None:
    # Codex 无 PostToolUse，正文内容网在回合结束的 Stop 事件兜底：对本回合 git 改动过的正文
    # 复扫硬信号（截断/拒绝语/工程词/复读）。命中时续开一回合修复；解析失败一律 {continue:True}。
    # Stop hooks require JSON on stdout.
    continuation_already_active = obj.get("stop_hook_active") is True
    try:
        root = project_root()
        blocks: list[str] = []
        ...
        if blocks:
            ...
            if continuation_already_active:
                emit({"continue": True, "systemMessage": reason})
            else:
                emit({
                    "decision": "block",
                    "reason": reason,
