...
def resolve_hook_cwd(value: str) -> Path | None:
    ...
    for env_name in ("CODEX_PROJECT_DIR",):
        ...
        if normalized != raw_normalized and not normalized.startswith(raw_normalized + "/"):
            continue
...
def read_active_book(root: Path) -> Path | None:
    ...
    if active_file.exists():
        ...
        active_rel = next((line.strip() for line in lines if line.strip()), "")
        if active_rel:
            candidate = (root / active_rel).resolve()
            try:
...
def command_base(root: Path) -> Path:
    if HOOK_CWD and _path_inside(root, HOOK_CWD):
        return HOOK_CWD.resolve()
    return root
...
def resolve_target(root: Path, target: str, base: Path | None = None) -> Path:
    ...
    if os.name == "nt" and normalized.startswith("/"):
        parent = resolve_hook_cwd(str(PurePosixPath(normalized).parent))
        if parent is not None:
            return (parent / PurePosixPath(normalized).name).resolve()
    p = Path(normalized)
    return p.resolve() if p.is_absolute() else ((base or root) / p).resolve()
...
def extract_prose_targets_from_command(command: str) -> list[str]:
    ...
    def add_match(m: re.Match[str]) -> None:
    ...
    for seg in re.split(r"[;&|\n]", command):
        ...
        try:
            ...
            words = seg.split()
        if len(words) >= 2 and words[0] in ("tee", "touch"):
            for word in words[1:]:
...
def target_paths_from_hook(obj: dict[str, Any]) -> list[Path]:
    ...
    target_specs: list[tuple[str, Path]] = []
    for key in ("file_path", "filePath", "path", "target", "filename"):
        value = tool_input.get(key)
        if isinstance(value, str):
            target_specs.append((value, root))
    command = tool_input.get("command")
    if isinstance(command, str):
        base = command_base(root) if tool_name in ("Bash", "shell_command", "functions.shell_command") else root
        target_specs.extend((t, base) for t in extract_apply_patch_targets(command))
        if tool_name in ("Bash", "shell_command", "functions.shell_command"):
            target_specs.extend((t, base) for t in extract_prose_targets_from_command(command))
        elif tool_name in ("apply_patch", "functions.apply_patch"):
            target_specs.extend((t, root) for t in extract_prose_targets_from_command(command))
    patch = tool_input.get("patch")
    if isinstance(patch, str):
        target_specs.extend((t, root) for t in extract_apply_patch_targets(patch))
    return [resolve_target(root, t, base) for t, base in target_specs if t]
