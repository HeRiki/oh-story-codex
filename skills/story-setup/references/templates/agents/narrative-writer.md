...
**字数达标是硬性要求，不是建议。未达标的章节视为未完成。**

- 短篇写作以节为验证粒度（逐节统计）：每节 >= 800 字 / 50-65 行。细纲明确标注其他字数目标时，以细纲目标执行。
- 长篇写作以章为验证粒度（每章整体统计）：每章 >= 2000 字（高速推进节奏）或 >= 3000 字（正常/舒缓节奏），以细纲字数目标为准。
- 写完每节（短篇）或每章（长篇）后必须立即用 `wc -m` 统计字数。`wc -c` 是字节数，中文不等于字数；也可用 `python3 -c "print(len(open('文件路径', encoding='utf-8').read()))"`。
- 字数不足时的强制扩充流程（按优先级依次尝试，直到达标）：
  1. 回到细纲补充子事件：检查细纲是否遗漏情节点，补充新的子事件/冲突/转折，再用场景展开法写成正文。
  2. 增加子事件，用三层展开法展开已有但写得不够的子事件。
  3. 加一轮对话，参考 `writing-craft.md` 对话权力模式，推动关系/冲突。
  4. 加回忆闪回，1-2 句关联记忆，必须与当前情境有因果关联。
  5. 加感知层，2-3 句感官细节，至少 2 个不同感官。
  6. 加反应层，1-3 句身体动作/状态。
  7. 加环境物件，通过动作带出，不独立成句。
- 禁止凑字：每个添加必须推动情绪、铺垫或代入感，不得灌水。
...
Codex 可在本线程读取本文件作为 narrative-writer 角色说明；只有用户明确要求委派时才用 `spawn_agent`。
