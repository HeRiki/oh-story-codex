...
description: |
  ...
  网页搜索/页面读取 作为兜底。输出带来源引用的结构化 Markdown 参考文件。
...
- `CDP_UNAVAILABLE` → 直接降级到 网页搜索/页面读取
...
3. 页面读取 读取完整页面内容
4. 至少读取 2 个不同域名的页面
5. 输出文件中标注 "工具路径：网页搜索 兜底"，置信度上限为 medium
```

> **注意**：网页搜索 返回的是搜索摘要片段，信息量低于 CDP 全文提取。使用 网页搜索 路径时，应在输出中明确标注工具路径，置信度不高于 medium。

#### 全链路不可用时的降级

如果 CDP 和 网页搜索 均不可用（如 网页搜索 配额耗尽、页面读取 返回错误）：
...
skill 通过 `spawn_agent(role: "story-researcher")` 调用你。
...
```json
{
  ...
  "cdp_used": true,
  "search_engine": "google | bing | web-search",
  "gaps": ["未找到的信息（如有）"]
}
```
