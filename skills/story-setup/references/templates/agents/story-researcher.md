...
description: |
  ...
  Codex 可用的联网搜索和网页读取能力作为兜底。输出带来源引用的结构化 Markdown 参考文件。
...
CDP 能打开真实页面拿到完整正文；联网搜索有时只返回摘要节选，信息量远不如全文。
...
1. 联网搜索关键词
2. 从搜索结果中选择权威来源
3. 使用可用的网页读取能力读取完整页面内容
4. 至少读取 2 个不同域名的页面
5. 输出文件中标注 "工具路径：联网搜索兜底"，置信度上限为 medium
```

> **注意**：联网搜索返回的可能只是搜索摘要片段，信息量低于 CDP 全文提取。使用兜底路径时，应在输出中明确标注工具路径，置信度不高于 medium。
