...
description: |
  ...
  联网搜索/网页读取 作为兜底。输出带来源引用的结构化 Markdown 参考文件。
...
CDP 能打开真实页面拿到完整正文；联网搜索 只返回摘要节选，信息量远不如全文。
...
- `CDP_UNAVAILABLE` → 直接降级到 联网搜索/网页读取
...
3. 网页读取 读取完整页面内容
4. 至少读取 2 个不同域名的页面
5. 输出文件中标注 "工具路径：联网搜索 兜底"，置信度上限为 medium
```

> **注意**：联网搜索 返回的是搜索摘要片段，信息量低于 CDP 全文提取。使用 联网搜索 路径时，应在输出中明确标注工具路径，置信度不高于 medium。

#### 全链路不可用时的降级

如果 CDP 和 联网搜索 均不可用（如 联网搜索 配额耗尽、网页读取 返回错误）：
...
skill 通过 `spawn_agent(name="story-researcher")` 调用你。
