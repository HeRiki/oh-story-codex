...
| 2 | 逐章摘要 | 章节文本 | 章节摘要.md（含情节点+角色）。角色过滤（龙套不提取、别名归类）。每章3-40情节点（密度150-200字/个，按字数动态调节）。**Codex 模式：读取 chapter-extractor 参考提示词逐章提取；默认本线程批处理，用户明确要求委派时才用 spawn_agent**。**计数验证：摘要数 == 章节数，不等则标记失败章节**。 | 所有章节处理完成 |
...
Stage 2 使用 `chapter-extractor` 角色提示词逐章处理，替代原来的固定分块摘要。

### 调用方式

1. 优先读取当前写作项目的 `.codex/story-agents/chapter-extractor.md`。
2. 如果项目尚未部署该文件，读取本 skill 的 `../story-setup/references/templates/agents/chapter-extractor.md` 作为参考提示词。
3. 默认在本线程按章节或按批次执行提取。只有用户明确要求多代理、并行委派或分工执行时，才使用 `spawn_agent`，并把 `chapter-extractor` 参考提示词作为子任务说明的一部分传入。

### 批量策略

- 每批处理 5-8 章，避免单轮输出过长。
- 每批完成后更新 `_progress.md`，记录已处理章节。
- 若使用 `spawn_agent`，等待当前批次完成后再进入下一批，避免上下文和文件写入混乱。

### 输出收集

- 每章输出 markdown 格式的提取结果。
- 主线程将输出写入 `章节/第{N}章_摘要.md`。
- 收集所有章节的出场人物表，供 Stage 3 合并使用。
...
- 全部批次完成后，对失败章节重试一次。
- 重试仍失败的章节标记为 `跳过`，在拆文报告中注明。

### 降级策略

如果 `chapter-extractor` 参考提示词不存在，Stage 2 退回主线程串行处理，但仍按每章 3-40 情节点的动态密度公式输出。

---

## 分块策略

Stage 2 使用 chapter-extractor 参考提示词按章节处理，不做固定分块。
...
- **输出长度上限**：Stage 2 按密度公式输出（每章情节点总量由字数动态决定）。Stage 3-5 串行分块模式下，单阶段输出不超过 8000 中文字符（用 `wc -m` 统计，超出时优先保留情节点和角色数据，缩写环境描写和心理分析）
