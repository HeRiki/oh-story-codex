...
优先读取 `.codex/story-agents/chapter-extractor.md` 作为章节提取角色说明。默认由主线程串行处理；只有用户明确要求多代理或并行拆解时，才使用 `spawn_agent` 按章委派，每次 5-8 个并发。
...
Stage 2 默认在主线程按章处理；用户明确要求并行时，可使用 chapter-extractor 参考提示词按章委派（每章一个子任务，每批 5-8 个），不分块。
