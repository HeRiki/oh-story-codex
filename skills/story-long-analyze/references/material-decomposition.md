...
优先使用 chapter-extractor 参考提示词处理每章。默认由主线程按章节或批次执行；只有用户明确要求多代理、并行委派或分工执行时，才使用 `spawn_agent`。
参考提示词不可用时，退回主线程串行处理。

输出格式严格对齐本阶段 A/B/C 三部分的合并输出，详见 output-templates.md 阶段2模板。
...
Stage 2 使用 chapter-extractor 参考提示词逐章处理（每批 5-8 章），不做固定分块。只有用户明确要求委派时才使用 `spawn_agent`。
...
输入分块大小 6-8K token/块（中小型），章节边界对齐。输出长度限制见 SKILL.md（Stage 2 按密度公式输出，Stage 3-5 单阶段 ≤8000 中文字符）。每块完成后更新 _progress.md。
