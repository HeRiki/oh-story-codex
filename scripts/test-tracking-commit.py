...
class TrackingCommitTests(unittest.TestCase):
    ...
    def test_check_compares_derived_views_to_state_without_parsing_markdown(self) -> None:
        ...
        self.run_tool("rebuild")
        self.run_tool("check")

    def test_rebuild_repairs_chapter_zero_views_without_advancing_state(self) -> None:
        self.init(last_chapter=0)
        tracking = self.project / "追踪"
        state_before = (tracking / "_tracking-state.json").read_bytes()
        (tracking / "上下文.md").write_text("# 损坏视图\n", encoding="utf-8")
        (tracking / "时间线/作者真相.md").unlink()
        (tracking / "角色状态/孤儿.md").write_text("# 孤儿\n", encoding="utf-8")

        rebuilt = self.run_tool("rebuild")

        self.assertEqual(json.loads(rebuilt.stdout)["last_committed_chapter"], 0)
        self.assertEqual((tracking / "_tracking-state.json").read_bytes(), state_before)
        self.assertFalse((tracking / "角色状态/孤儿.md").exists())
        self.assertFalse(any((tracking / "逐章记录").glob("第*章.md")))
        self.run_tool("check")
