...
[ -n "$PYBIN" ] || fail "Python 3.10+ is required for Codex CLI E2E"
...
if ! ln -s ../skills "$WORKSPACE/.agents/skills" 2>/dev/null || [ ! -d "$WORKSPACE/.agents/skills" ]; then
  rm -rf "$WORKSPACE/.agents/skills"
  cp -R "$WORKSPACE/skills" "$WORKSPACE/.agents/skills"
fi
...
allowed_roots = (repo_root / "skills", repo_root / ".agents/skills")
allowed_paths = {
    name: [locator(root / name / "SKILL.md") for root in allowed_roots]
    for name in expected
}
missing = sorted(
    name
    for name, paths in allowed_paths.items()
    if not any(path in rendered for path in paths)
)
if missing:
    raise SystemExit(f"Codex prompt input omitted repository skills: {missing}")

repo_pattern = re.compile(
    re.escape(locator(repo_root)) + r"/(?:skills|\.agents/skills)/([^/]+)/SKILL\.md"
)
