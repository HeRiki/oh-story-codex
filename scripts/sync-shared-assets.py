...
def load_groups(root: Path, manifest_path: Path) -> list[Group]:
    ...
    for index, raw_group in enumerate(raw_groups):
        ...
        if previous_source_owner:
            raise ManifestError(
                ...
                f"{rel(root, source)} in {previous_source_owner} and {name}"
            )
        ...
        if len(set(targets)) != len(targets):
            ...
            raise ManifestError(
                f"duplicate managed target {rel(root, duplicate)} repeated in {name}"
            )
        for target in targets:
            previous = target_owners.get(target)
            if previous:
                raise ManifestError(
                    f"duplicate managed target {rel(root, target)} in {previous} and {name}"
                )
            if target.name != source.name:
                raise ManifestError(
                    f"{name}: target {rel(root, target)} must keep canonical "
    ...
    if source_targets:
        ...
        raise ManifestError(
            f"managed path {rel(root, path)} is both source for "
...
def run(command: str, root: Path, groups: list[Group]) -> int:
    ...
    for group in groups:
        if not group.source.is_file():
            print(f"MISSING SOURCE [{group.name}] {rel(root, group.source)}")
            issues += 1
            continue
        try:
            ...
            print(
                f"SOURCE ERROR [{group.name}] {rel(root, group.source)}: {exc}"
            )
            issues += 1
            continue
