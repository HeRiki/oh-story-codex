...
describe("HTTP API", () => {
  ...
  test(
    "keeps the original file when the Windows atomic replacement helper cannot start",
    { skip: process.platform !== "win32" && "仅 Windows 使用 PowerShell 原子替换 helper" },
    async () => {
      const root = await createWorkspace();
      const target = resolve(root, "长篇/示例书/正文/第001章.md");
      const info = await stat(target);

      await assert.rejects(
        replaceFileAtomically(target, "绝不能截断原稿", info.mode, {
          platform: "win32",
          powershellExecutable: `missing-powershell-${Date.now()}.exe`,
        }),
      );

      assert.equal(await readFile(target, "utf8"), "初稿");
      const leftovers = await readdir(resolve(root, "长篇/示例书/正文"));
      assert.equal(leftovers.some((name) => name.endsWith(".tmp")), false);
    },
  );
